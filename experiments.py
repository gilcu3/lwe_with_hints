from lwe_with_hints import LWELattice, generateToyInstance, generateLWEInstance
...
def parseArguments():
    ...
    parser.add_argument(
        ...
        type=str,
        default="0:0:1",
        help='String of the form "min:max:step" specifying the number of hints to integrate.',
    )
    parser.add_argument(
        "-trials", type=int, default=1, help="Number of instances to generate."
    )
    parser.add_argument(
        "-file",
        type=str,
        default="output_experiments.txt",
        help="Output is written into file with name -file.",
    )
    parser.add_argument(
        "-hints_centered",
        action="store_true",
        help="If set, then hints are drawn from {-(q-1)/2,...,(q-1)/2}^n. Otherwise from {0,...,q-1}^n.",
    )
    parser.add_argument(
        "-modular",
        action="store_true",
        help="If set, then generate mod-q hints. Otherwise perfect hints.",
    )
...
def generateHints(s, q, k, centered):
    ...
    for i in range(k):
        if centered:
            v = np.array(
                [randrange(-int((q - 1) / 2), int((q + 1) / 2)) for _ in range(n)]
            )
        else:
...
def experiment(A, b, q, hints, modular, fileName, verbose):
    ...
    try:
        ...
        output = "Finished experiment.\tHints: %d\tBlocksize: %d\tTime: %fs" % (
            ctrHints,
