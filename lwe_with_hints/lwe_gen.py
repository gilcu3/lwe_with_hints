...
def kyberGen(variant):
    if variant not in [512, 768, 1024]:
        raise NotImplementedError(
            "kyberGen(variant) supports only variant = 512, 768, 1024, but variant = %d was given."
            % variant
        )
...
def dilithiumGen(variant):
    if variant not in [2, 3, 5]:
        raise NotImplementedError(
            "dilithiumGen(variant) supports only variant = 2, 3, 5, but variant = %d was given."
            % variant
        )
...
def falconGen(n):
    if n not in [2**i for i in range(1, 11)]:
        raise NotImplementedError(
            "falconGen(n) supports only n = 2, 4, ..., 1024 but n = %d was given." % n
        )
...
def ntruGen(variant):
    if variant not in ["HPS-509", "HPS-677", "HPS-821", "HRSS"]:
        raise NotImplementedError(
            "ntruGen(variant) supports only variant = HPS-509, HPS-677, HPS-821, HRSS but "
            + variant
            + " was given."
        )
